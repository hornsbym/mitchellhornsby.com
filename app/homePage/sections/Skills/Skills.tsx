...
import { RiAddLine, RiLoader2Line, RiSubtractLine } from "react-icons/ri";
...
export default function SkillsSection() {
    const { loading, error, data } = useQuery(GET_SKILL_IDS);
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-4 gap-y-8">
                            {
                                loading
                                    ...
                                    : error
                                        ...
                                        : (data.Skills.docs.map((sk, i) => <SkillTile key={`${sk.skill}-${i}`} {...sk} />))
                            }
                        </div>
                    </div>
                </div>
...
// }: SkillGaugeProps) => {
//     return (
//         <div className={`flex flex-col justify-between p-4 bg-zinc-900 dark:bg-white rounded-md w-fit dm-text-inverse gap-4 drop-shadow-light dark:drop-shadow-dark`}>
